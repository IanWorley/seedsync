...
export class FileOptionsComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this._viewFileService.files.subscribe((files) => {
            this.isExtractedStatusEnabled =
                FileOptionsComponent.isStatusEnabled(
                    files,
                    ViewFile.Status.EXTRACTED
                );
            this.isExtractingStatusEnabled =
                FileOptionsComponent.isStatusEnabled(
                    files,
                    ViewFile.Status.EXTRACTING
                );
            this.isDownloadedStatusEnabled =
                FileOptionsComponent.isStatusEnabled(
                    files,
                    ViewFile.Status.DOWNLOADED
                );
            this.isDownloadingStatusEnabled =
                FileOptionsComponent.isStatusEnabled(
                    files,
        ...
        this.viewFileOptionsService.options.subscribe(
            (options) => (this._latestOptions = options)
        );
    }
    ...
    onToggleShowDetails() {
        this.viewFileOptionsService.setShowDetails(
            !this._latestOptions.showDetails
        );
    }

    onTogglePinFilter() {
        this.viewFileOptionsService.setPinFilter(
            !this._latestOptions.pinFilter
        );
    }

    private static isStatusEnabled(
        files: Immutable.List<ViewFile>,
