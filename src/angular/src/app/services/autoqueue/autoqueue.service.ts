...
export class AutoQueueService extends BaseWebService {
    ...
    private readonly AUTOQUEUE_ADD_URL = (pattern) =>
        `/server/autoqueue/add/${pattern}`;
    private readonly AUTOQUEUE_REMOVE_URL = (pattern) =>
    ...
    public add(pattern: string): Observable<WebReaction> {
            ...
            return Observable.create((observer) => {
                observer.next(
                    new WebReaction(
                        false,
                        null,
                        Localization.Notification.AUTOQUEUE_PATTERN_EMPTY
                    )
                );
            });
        }
        ...
        const index = currentPatterns.findIndex(
        ...
        if (index >= 0) {
            ...
            const patternEncoded = encodeURIComponent(
            ...
            obs.subscribe({
                next: (reaction) => {
                    if (reaction.success) {
                        // Update our copy and notify clients
                        const patterns = this._patterns.getValue();
                        const finalIndex = currentPatterns.findIndex(
                            (pat) => pat.pattern === pattern
                        );
    ...
    private getPatterns() {
        ...
        this._restService.sendRequest(this.AUTOQUEUE_GET_URL).subscribe({
            next: (reaction) => {
                if (reaction.success) {
                    const parsed: AutoQueuePatternJson[] = JSON.parse(
                        reaction.data
                    );
...
export let autoQueueServiceFactory = (
    ...
    const autoQueueService = new AutoQueueService(
        _streamServiceRegistry,
        _restService,
        _logger
    );
